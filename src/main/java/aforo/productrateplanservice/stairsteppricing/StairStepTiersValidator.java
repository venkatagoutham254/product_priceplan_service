...
public class StairStepTiersValidator implements ConstraintValidator<ValidStairStepTiers, StairStepPricingCreateUpdateDTO> {
    @Override
    public boolean isValid(StairStepPricingCreateUpdateDTO dto, ConstraintValidatorContext context) {
        List<StairStepTierCreateUpdateDTO> tiers = dto.getTiers();
        if (tiers == null || tiers.isEmpty()) return true; // @NotNull/@NotEmpty handled elsewhere

        int prevEnd = -1;
        Set<Integer> seenStarts = new HashSet<>();
        
        for (StairStepTierCreateUpdateDTO tier : tiers) {
